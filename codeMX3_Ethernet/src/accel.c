...
uint32_t compteur_interrupt=0;
uint32_t compteur_interrupt2=0;//compteur pour vérifier la fréquence d'acquisition des donneés
uint32_t compteur_packet=0;
char accel_packet[484];
bool flag_packet;
uint32_t count = 0;        
void ACL_Init()
{
    ...
    ACL_SetRegister(ACL_CTRL_REG1, 0b00001001);        // Output data rate at 800Hz, no auto wake, no auto scale adjust, no fast read mode
    ACL_SetRegister(ACL_CTRL_REG1, 0b00001001);  
}
...
void accel_tasks()
{
    ...
    if(SWITCH1StateGet())
    {
        ...
        char outbuf[80];
//        sprintf(outbuf, "X: %06x", compteur_packet);
...
//        SSD_WriteDigitsGrouped(compteur_interrupt2, 0x1);
        accel_data_ready = false;
    }
}

//void createAccelPacket()
//{
//    
//}
...
extern int ACL_SignedInt(unsigned char *rgVals)
{
    return abs(((signed int)rgVals[0] << 24) >> 20 | (rgVals[1] >> 4));
}
